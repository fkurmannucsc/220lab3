  ...
  try:
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        IPC = 0
        with open(exp_path+config_key+'/inst.stat.0.csv') as f:
          lines = f.readlines()
          for line in lines:
            if 'ST_OP_NOP_pct' in line:
    ...
    plot_data(benchmarks, ipc, 'NOP Ratio', output_dir+'/FigureNOP.png')
...
def plot_data(benchmarks, data, ylabel_name, fig_name, ylim=None):
  ...
  key_ind = np.arange(len(data.keys()))
  width = 0.18
  ...
  for key in data.keys():
    ...
    if idx % 2:
      ...
      hatch='///'
    ax.bar(idx, data[key], width=width*5, fill=False, hatch=hatch, color=colors[idx], edgecolor=colors[idx])
    idx += 1
  ax.set_xlabel("Configurations")
  ax.set_ylabel(ylabel_name)
  ax.set_xticks(key_ind)
  ax.set_xticklabels(data.keys(), rotation = 27, ha='right')
  ax.grid('x');
  if ylim != None:
    ax.set_ylim(ylim)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o','--output_dir', required=True, help='Output path. Usage: -o /home/$USER/plot/lab3')
